...
export function globalFlag(szData, team, debug = false) {
  const statisticsFlagArr = statisticsFlagList.map(({ name, regex, handler }) => {
    ...
    return { name, flag: handler(szData, team) };
  });

  logDebug({
    module: 'statisticsFlagList', szData, team, statisticsFlagArr,
  }, debug);
  ...
  logDebug({
    module: 'questionConfigList', szData, team, questionConfigFlagArr,
  }, debug);
  ...
  logDebug({
    module: 'chainEffectList', szData, team, chainEffectFlagArr,
  }, debug);

  return [...statisticsFlagArr, ...questionConfigFlagArr, ...chainEffectFlagArr]
    .map(({ flag }) => flag)
    .filter((flag) => !!flag)
...
export function groupGlobal(globalFlags, effect) {
  ...
  if (effect.chainSecure !== undefined) {
    ...
    nextGlobalFlags.chainSecure = Math.max(globalFlags.chainSecure, effect.chainSecure);
  }

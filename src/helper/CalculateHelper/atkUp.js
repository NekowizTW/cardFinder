...
const atkUpList = [
  ...
  },
  {
    ...
    handler: ({ elmts = ['火', '水', '雷'], const: val }, team) => {
      const effect = initEffect(team.length, 'atk');
      effect.target = team.map((card) => !!card.prop && elmts.includes(card.prop) >= 0);
      effect.atk += strToInt(val);
      return effect;
    },
  },
  ...
  },
  {
    ...
    handler: ({ elmts, const: val }, team) => {
      const effect = initEffect(team.length, 'atk');
      effect.target = team.map((card) => !!card.prop2 && elmts.includes(card.prop2));
      effect.atk += strToInt(val);
      return effect;
    },
  },
  ...
  },
  {
    ...
    handler: ({ elmts, const1: val1, const2: val2 }, team) => {
      // main prop effect (it should be add at: increase atk of specified main prop)
  ...
  },
  {
    ...
    handler: ({ info, const: val }, team) => {
      ...
      effect.target = team.map(
        (card) => !!card.breed && affectBreeds.includes(card.breed),
      );
      return effect;
    },
  },
...
export default function atkUp(szData, team, tar, debug = false) {
  const effectArr = atkUpList.map(({ name, regex, handler }) => {
    ...
    logDebug(`atkUp/${name}`, effect, debug);
  ...
  const statistics = effectArr
    ...
    .filter((effect) => !Number.isNaN(effect.atk));

  logDebug('atkUp/statistics', { szData, team, statistics }, debug);

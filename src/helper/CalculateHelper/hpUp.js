...
const hpUpList = [
  ...
  },
  {
    ...
    handler: ({ elmts, const1: val1, const2: val2 }, team) => {
      // main prop effect (it should be add at: increase hp of specified main prop)
...
export default function hpUp(szData, team, tar, debug = false) {
  const effectArr = hpUpList.map(({ name, regex, handler }) => {
    ...
    logDebug(`hpUp/${name}`, effect, debug);
  ...
  const statistics = effectArr
    ...
    .filter((effect) => !Number.isNaN(effect.hp));

  logDebug('hpUp/statistics', { szData, team, statistics }, debug);

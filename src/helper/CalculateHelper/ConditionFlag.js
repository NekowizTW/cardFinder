...
export function conditionFlag(szData, team, tar, debug = false) {
  const flagArr = conditionalFlagList.map(({ name, regex, handler }) => {
    ...
    logDebug(`conditionFlag/${name}`, flag, debug);
  ...
  logDebug('conditionFlag/statistics', { szData, team, statistics }, debug);
...
export function skillRelative(szData, team, tar, debug = false) {
  ...
  const asFlagArr = asRelativeList.map(({ name, regex, handler }) => {
    ...
    logDebug(`skillRelative/asUp/${name}`, flag, debug);
  ...
  const ssFlagArr = ssRelativeList.map(({ name, regex, handler }) => {
    ...
    logDebug(`skillRelative/ssUp/${name}`, flag, debug);

    return { name, flag };
  });

  const statistics = [...asFlagArr, ...ssFlagArr]
    .map(({ flag }) => flag)
    .filter((flag) => !!flag)
    .flatMap((flag) => (Array.isArray(flag) ? flag : [flag]));

  logDebug('skillRelative/statistics', { szData, team, statistics }, debug);
